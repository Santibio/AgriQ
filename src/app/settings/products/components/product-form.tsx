...
export default function ProductForm({ product }: ProductFormProps) {
  ...
  const typesForCategory = useMemo(() => {
    ...
    return (
      config.productCategories.find(cat => cat.id === selectedCategory)?.type ||
      []
    )
  }, [selectedCategory])
  ...
  return (
    ...
    >
      <div className='pb-1 flex-1 w-full flex gap-4 flex-col'>
        <Controller
          name='name'
          control={control}
          render={({ field }) => (
        ...
        <Controller
          ...
          render={({ field: { onChange, onBlur, ref, value } }) => {
            ...
            return (
              <label
                className={`flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer text-gray-500 hover:bg-slate-100 hover:dark:bg-slate-900 ${
              ...
              >
                <div className='flex flex-col items-center justify-center pt-5 pb-6'>
                  ...
                  {value?.name && (
                    <p className='text-xs text-gray-500'>File: {value.name}</p>
                  )}
                </div>
                <input
                  type='file'
